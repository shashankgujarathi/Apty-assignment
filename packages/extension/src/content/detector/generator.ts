...
function isStableId(id: string): boolean {
  if (!id) return false;
  const dynamicRegex =
    /(^|\b)(react-aria-|ember-|vue-|id-|_ng|btn-|lnk-).*?[0-9]{3,}/i;
...
function filterStableClasses(classList: DOMTokenList): string[] {
  ...
  const tailwindPattern =
    /^(m|p|w|h|bg|text|border|flex|grid|justify|items|self|rounded|shadow|opacity|cursor|transition|duration|hover|focus|active|z|top|left|right|bottom|translate|scale|rotate)-/;
...
export function generateSelectors(element: HTMLElement): ElementSelectors {
  ...
  const classSelector =
    stableClasses.length > 0 ? `.${stableClasses.join(".")}` : undefined;
